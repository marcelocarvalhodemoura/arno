import { findType, type SuggestedTx, type StatementCatalog } from "./statement.js";
...
function applyHit(row: SuggestedTx, hit: AiHit | undefined, catalog: StatementCatalog): SuggestedTx {
  ...
  return {
    ...
    memberGuardianId:
      member?.guardians?.find((item) => foldName(row.description).includes(foldName(item.name)) && item.name.length >= 5)
