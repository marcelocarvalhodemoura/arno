...
export function cashFlow(
  ...
  const months = monthsBetween(from, to).map((month) => {
    ...
    const txs = db.transactions.filter(
      (t) => isSettled(t) && t.date.startsWith(prefix) && inRange(t.date, from, to),
    );
    const income = sumBy(
...
export function customReport(
  ...
  const ledger: FiscalLedgerLine[] = txs.map((t, index) => {
    ...
    const account = t.memberAccountId
      ? db.memberAccounts.find((a) => a.id === t.memberAccountId)
      : undefined;
...
export function dashboard(db: DatabaseShape, year: number, month: number): DashboardPayload {
  ...
  const chart = monthsBetween(yearFrom, yearTo).map((key) => {
    const txs = db.transactions.filter((t) => isSettled(t) && t.date.startsWith(key));
    return {
      month: key,
      income: sumBy(
        txs.filter((t) => t.type === "income"),
