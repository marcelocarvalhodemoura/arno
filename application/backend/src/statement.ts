import type { BranchId, PaymentMethod, TxNature, TxPaymentStatus, TxType } from "./types.js";
import { fold, parseCsv, parseIsoDate, parseSignedAmount, pick } from "./csv.js";
...
const KEYWORDS: { needles: string[]; typeName: string; direction?: TxType }[] = [
  ...
  { needles: ["energisa", "copel", "rge", "energia", "internet", "sanepar", "utilidade"], typeName: "Utilidades", direction: "expense" },
  { needles: ["aluguel", "condominio", "iptu", "sede"], typeName: "Sede", direction: "expense" },
  { needles: ["ifood", "mercado", "supermercado", "padaria", "lanche", "restaurante", "aliment"], typeName: "Alimentação", direction: "expense" },
...
function fromTemplate(row: Record<string, string>, line: number, catalog: StatementCatalog): SuggestedTx | null {
  ...
  const guardianHit = member
    ? matchGuardian(guardianName || memberName, member)
    : undefined;
  return {
    line,
    date: date ?? "",
...
function fromBank(row: Record<string, string>, line: number, catalog: StatementCatalog): SuggestedTx | null {
  ...
  const flagged = parseDirection(
    pick(row, "tipo", "dc", "c_d", "natureza", "entrada_saida", "credito_debito"),
  );
  const inferred = inferBankDirection(description);
  const type: TxType | null =
    flagged ?? inferred ?? (signed < 0 ? "expense" : signed > 0 ? "income" : null);
  ...
  const memberHit = matchMember(description, catalog.members.filter((item) => item.status !== "inactive"));
  ...
  if (!movement && memberHit && type === "income") {
    const feeMatch = feeNameForAmount(amount, catalog.fees) ?? (near(amount, memberHit.member.monthlyFee) ? "Mensalidade" : null);
    if (feeMatch) {
  ...
  if (!movement && type === "income" && memberHit && near(amount, memberHit.member.monthlyFee)) {
  ...
  return {
    ...
    confidence,
    hint: memberHit
      ? `${hint} · ${memberHit.member.name}${guardian ? ` · ${guardian.name}` : ""}`
      : hint,
...
function classifyMovement(
  description: string,
  direction: TxType,
  types: StatementCatalog["movementTypes"],
) {
  const key = fold(description);
  ...
  const named = types
    .filter((item) => item.active && item.name.length >= 4 && (item.direction === "both" || item.direction === direction))
...
function findPendingFee(
  ...
  const matches = (catalog.pendingPayments ?? []).filter(
    (item) => item.memberId === memberId && near(item.amount, amount),
  );
