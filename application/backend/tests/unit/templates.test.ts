...
import type { DatabaseShape, Member, Transaction } from "../../src/shared/types.js";
...
describe("charge email template", () => {
  ...
  it("shows the group logo, CNPJ Pix key and finance WhatsApp", () => {
    expect(existsSync(logoFilePath())).toBe(true);
    ...
    expect(message.html).toContain(`cid:${LOGO_CID}`);
    expect(message.html.split(`cid:${LOGO_CID}`).length - 1).toBeGreaterThanOrEqual(2);
