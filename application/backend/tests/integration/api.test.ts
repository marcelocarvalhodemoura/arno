...
import { isUuidV7 } from "../../src/shared/id.js";
import { verifyPassword } from "../../src/shared/auth/password.js";
import { rehashLegacySeedUsers } from "../../src/identity/users.js";
import { invalidateCache } from "../../src/shared/persistence/finance-store.js";
...
async function passwordHashOf(username: string): Promise<string> {
  const result = await pool.query<{ password_hash: string }>("SELECT password_hash FROM users WHERE username = $1", [
    username,
  ]);
...
async function ensureType(auth: { Authorization: string }, name: string, direction: "income" | "expense" | "both") {
  ...
  const created = await request(app).post("/api/movement-types").set(auth).send({ name, direction, description: name });
...
describe("API integration", () => {
  ...
  it("lets the tesoureiro use cash flow endpoints", async () => {
    ...
    const patchedMember = await request(app).patch(`/api/members/${createdMember.body.id}`).set(auth).send({
    ...
    const duplicate = await request(app).post("/api/members").set(auth).send({
    ...
    const flow = await request(app).get("/api/reports/cashflow?from=2026-08-01&to=2026-08-31").set(auth);
  ...
  it("lets tesoureiro split a credit and read settings", async () => {
    ...
    const dueDay = await request(app).patch("/api/settings").set(auth).send({ mensalidadeDueDay: 15 });
    expect(dueDay.status).toBe(200);
    expect(dueDay.body.mensalidadeDueDay).toBe(15);

    const blocked = await request(app).patch("/api/settings").set(auth).send({ groupName: "Não pode" });
    expect(blocked.status).toBe(403);

    const restore = await request(app).patch("/api/settings").set(auth).send({ mensalidadeDueDay: 10 });
    expect(restore.status).toBe(200);
    expect(restore.body.mensalidadeDueDay).toBe(10);

    const status = await request(app).get("/api/notify/status").set(auth);
  ...
  it("lets tesoureiro manage fees", async () => {
    ...
    const patched = await request(app).patch(`/api/fees/${created.body.id}`).set(auth).send({ amount: 40 });
    expect(patched.status).toBe(200);
...
`,
  ...
  it("reads a Sicredi PDF, matches the associate and marks pending mensalidade as paid", async () => {
    ...
    if (
      ...
      const account = await request(app).post(`/api/members/${memberId}/accounts`).set(auth).send({
    ...
    const pending = await request(app).post("/api/transactions").set(auth).send({
      date: "2026-01-10",
      type: "income",
      nature: "fixed",
  ...
  it("builds the March-December mensalidade grid and launches pending cash-flow rows", async () => {
    ...
    const first = await request(app).get("/api/mensalidades?year=2026").set(auth);
    expect(first.status).toBe(200);
    ...
    const again = await request(app).get("/api/mensalidades?year=2026").set(auth);
    expect(again.status).toBe(200);

    const listed = await request(app).get("/api/transactions?from=2026-05-01&to=2026-12-31").set(auth);
    ...
    const afterLeave = await request(app).get("/api/mensalidades?year=2026").set(auth);
    ...
    const remaining = await request(app).get("/api/transactions?from=2026-05-01&to=2026-12-31").set(auth);
  ...
  it("lets the admin read dashboard, users and projects", async () => {
    ...
    const withoutCurrent = await request(app)
    ...
    const patched = await request(app).patch(`/api/users/${created.body.id}`).set(auth).send({
    ...
    const report = await request(app).post("/api/reports/custom").set(auth).send({
      from: "2026-08-01",
      to: "2026-08-31",
      branches: [],
      types: [],
  ...
  it("stores passwords as bcrypt and upgrades the old scrypt hashes on login", async () => {
    ...
    await pool.query("UPDATE users SET password_hash = $2 WHERE username = $1", [TEST_TREASURER_USER, legacy]);
  ...
  it("syncs mock Sicredi Pix into cash flow and is idempotent", async () => {
    ...
    await pool.query("DELETE FROM transactions WHERE origin = 'sicredi' OR external_id IS NOT NULL");
    await pool.query("TRUNCATE bank_movements, bank_sync_state");
    invalidateCache();
    const auth = await tesoureiroAuth();
    await ensureType(auth, "Mensalidade", "income");
    ...
    const status = await request(app).get("/api/integrations/sicredi").set(auth);
    expect(status.status).toBe(200);
    expect(status.body.configured).toBe(true);
    expect(status.body.mock).toBe(true);

    const sync = await request(app).post("/api/integrations/sicredi/sync").set(auth).send({});
    ...
    const again = await request(app).post("/api/integrations/sicredi/sync").set(auth).send({});
    expect(again.status).toBe(200);
    expect(again.body.created).toBe(0);
    expect(again.body.paid).toBe(0);

    const denied = await request(app).post("/api/integrations/sicredi/webhook").send({ pix: [] });
    expect(denied.status).toBe(401);
    const webhook = await request(app).post("/api/integrations/sicredi/webhook?token=teste-webhook").send({ pix: [] });
  ...
  it("validates and receives WhatsApp webhooks without auth", async () => {
    ...
    const ready = await request(app).get("/webhook");
    expect(ready.status).toBe(200);
    expect(ready.body).toEqual({ ok: true, service: "whatsapp-webhook" });

    const denied = await request(app).get("/webhook").query({
      "hub.mode": "subscribe",
    ...
    const verified = await request(app).get("/webhook").query({
      "hub.mode": "subscribe",
    ...
    const verifiedApi = await request(app).get("/api/integrations/whatsapp/webhook").query({
      "hub.mode": "subscribe",
      "hub.verify_token": "teste-whatsapp",
      "hub.challenge": "abc",
    });
    ...
    const unknown = await request(app).post("/webhook").send({ object: "page" });
  ...
  it("rehashes seed users to bcrypt on startup when ADMIN_PASSWORD is set", async () => {
    const salt = randomBytes(16).toString("hex");
    const derived = (await scryptAsync(TEST_PASSWORD, salt, 64)) as Buffer;
    await pool.query("UPDATE users SET password_hash = $2 WHERE username = ANY($1)", [
  ...
  it("enqueues bulk mensalidade charges and the outbox worker delivers them", async () => {
    ...
    const grid = await request(app).get("/api/mensalidades?year=2026").set(auth);
    ...
    const { processOutbox } = await import("../../src/notifications/outbox.js");
    await processOutbox();

    const log = await request(app).get("/api/notify/log?limit=40").set(auth);
