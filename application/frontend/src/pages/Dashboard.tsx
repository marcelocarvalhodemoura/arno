...
import { BRANCH_LABELS, type BranchId, type DashboardPayload } from "@shared";
...
import { brl, chartMoney, MONTHS } from "../lib/format";
...
export default function Dashboard() {
  ...
  const { data, loading, error } = useFetch<DashboardPayload>(
  ...
  const periodLabel = month ? `${MONTHS[month - 1]} de ${year}` : `ano de ${year}`;
  const chart = data.chart.map((row) => ({
    name: MONTHS[Number(row.month.slice(5)) - 1]?.slice(0, 3) ?? row.month,
  ...
  return (
    ...
    <div>
      <PageHeader
        ...
        subtitle={`Arrecadação, associados cadastrados até o fim do período e o caixa do grupo em ${periodLabel}.`}
      />
      ...
      <div className="grid-stats">
        ...
        <StatCard
          title="Saldo inicial"
          value={brl(data.opening)}
          hint="Antes do período filtrado"
        />
      ...
      <div className="split-2" style={{ marginTop: 16 }}>
        <article className="card chart-card">
          <h3 style={{ marginBottom: 12 }}>Arrecadação {year}</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={chart}>
              ...
              <Legend />
              <Bar dataKey="Arrecadação" fill="#2d8a4e" radius={6} />
        ...
        <article className="card chart-card">
          ...
          <p className="muted" style={{ marginBottom: 12 }}>
            Filhotes, Lobinho, Escoteiro, Sênior, Pioneiro e Grupo no período.
          </p>
      ...
      <article className="card" style={{ marginTop: 16 }}>
        ...
        <p className="muted" style={{ marginTop: 12 }}>
          * Associados com data de cadastro até {data.to.split("-").reverse().join("/")}, conforme o filtro de mês e
          ano. O Clube da Flor de Lis aparece no cadastro e no caixa; o painel soma a arrecadação de Filhotes, Lobinho,
          Escoteiro, Sênior, Pioneiro e Grupo.
        </p>
      </article>
    </div>
    </FetchOverlay>
